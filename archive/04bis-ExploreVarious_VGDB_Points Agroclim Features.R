...
setwd("E:/Benjamin/_Recherches_Dijon/_AAA_TerraClimate/2023_WWV_ANALYSIS/")
  ...
  # Read data sets ---------
d8110 <- cbind(period="2001-2020",as.data.frame(fread("AgroIndices_CLIMOBS19812010_VGDB_Pts_v1.2.3.csv")))
...
d <- rbind(d8110[,SameNames], dobs[,SameNames],dfut[,SameNames])
...
d <- as_tibble(d) %>% left_join(kop[,c("peelcode", "code","niv.1")], by = join_by(KOPPEN_Avg==peelcode) )
unique(d$scenario)

cols <- c("GSP49_Avg", "GST49_Avg")

# Count by large climate classe
d %>% filter(scenario=="CLIMOBS19812010") %>% 
...
# Proportion of regions in a specific class
d %>% filter(scenario=="CLIMOBS19812010", code=="Dwa") %>% 
  group_by(CN_REG) %>%   
  summarise(n = n()) %>%
  mutate(freq = n / sum(n)*100) %>%
  arrange(desc(n) ) %>%
...
d %>% filter(scenario=="OBS", CN_REG %in% myregs) %>% 
...
d %>% filter(scenario=="OBS") %>% summarise(GST_Avg=min(GST_Avg), GST49_Avg=min(GST49_Avg))
