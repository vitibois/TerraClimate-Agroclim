# 2023-12-20 B. Bois
# Statistics of potential areas for viticulture
library(terra)
library(ggplot2)
...
####### CHARGEMENT DES FICHIERS ###########
  # Zones potentielles
obs <- rast("rasters_zonespotentielles/AAA_ZonesPotentielles_OBS2001-2020.tif")
p2c <- rast("rasters_zonespotentielles/AAA_ZonesPotentielles_plus2C.tif")
p4c <- rast("rasters_zonespotentielles/AAA_ZonesPotentielles_plus4C.tif")
pot <- rast(list(obs=obs,plus2C=p2c,plus4C=p4c))
time(pot) <- NULL

  # Superficie des zones potentielles
...
plot(pot)
  ...
  # RASTER HYBRIDE REUNISSANT TOUTES LES OPTIONS
# Code : 1 = climate vavorable et 0 climate défavorable
# TROIS CHIFFRES le premier est pour le climat +4°C, le Second pour le cliamt +2°C
# et le troisième pour le climat actuel 2001-2020
rpot <- potbin$obs
...
plotRGB(palege)
  ...
  # DEM monde
dem <- rast("E:/SIG/_MNT/SRTM_10minarc_WorldClim/alt.bil")
...
####### CALCUL ET GRAPHS DES SUPERFICIES POTENTIELLES ###########
  # Superficie totale des pixels dans lesquels on identifie de la vigne -----
global(rvpts,  "sum", na.rm=T)*100/10^6 # 23 Mha...bien plus grand que le vignoble réel, de 7.3 Mha (OIV 2022)
...
areavgbd <- areavgbd[order(areavgbd$area, decreasing=T),]
  ...
  # Superficies de la VGDB (vignes actuelles) à climat favorable ----
  # zones potentielles
vgdbpot <- rvpts*pot
...
vgdbarea
  ...
  # Total par pays et par type de potentialités -----
...
plot(potbin)
  ...
  # Calcul des superficies par pays des zones par période 
names(pot)
...
continent_area <- continent_area[order(continent_area$areapot_all21st,decreasing=T),]
  ...
  # Sauvegardes des tableaux
area <- area[order(area$areapot_all21st, decreasing=T),]
head(area)
write.csv(area,"out_tables/Superficies_ZonesPotentielles_km2_VignoblesActuelsInclus.csv", row.names=T)

  

#### Graph des superficies disponibles selon les périodes considérées
...
pdf("out_plots/Continent_areas_per_period.pdf", height = 5, width=8)
...
### Graph des superficies par continent
names(area)
pdf("out_plots/Bar_Area_Per_Continent.pdf", width = 7, height = 5)
...
pdf("out_plots/Bar_Area_Per_Country.pdf", width = 10, height = 6)
...
dev.off()  
  ...
  # Elimination des zones actuellement plantées en vigne
narvpts <- is.na(rvpts)
#narvpts[!narvpts] <- NA
...
pot2 <- pot * narvpts
plot(potbin)
  # Calcul de la superficie par pays
...
##### ANALYSES DU "VIGNOBLES ACTUELS" (VGDB POINTS L. DELELEE) ####
  # Cartes des vignobles actuels présentant ou non les criteres climatiques selectionnees
pdf("outmaps/VignoblesActuels_HorsCriteres.pdf", width = 7, height = 4)
  myfun <- function(){
...
##### ZONES POTENTIELLES - VIGNES "ACTUELLES" INCLUSES ######
# Calcul des superficies 
novpts <- is.na(rvpts)

  # Carte illustation les zones viticoles potentielles dans le monde
pdf("outmaps/CartesMondiale_ZonesPotentielles_VignoblesActuelsInclus.pdf", width = 10,height = 6)
...
dev.off()
  ...
  # Cartes des zones potentielles par région
pdf("outmaps/Regional_ZonesPotentielles_VignoblesActuelsInclus.pdf", width=12, height = 6)
...
# Vecteur des zones non-viables à terme (pour hachurage)
...
myext
pdf("outmaps/AAA_Global_and_Regional_ZonesPotentielles2040AllWines_VignoblesActuelsInclus.pdf", width=12, height = 6)
...
pdf("outmaps/AAA_Global_and_Regional_ZonesPot-2001-2100-AllWines_VignoblesActuelsInclus.pdf", width=12, height = 6)
  mymaxcell <- 40*10^6
  #mymaxcell <- 10^6
  #myext <- ext(unlist(zoomtab[3,-1]))
...
writeRaster(potbin$all21st,"AAA_ZonesPotentielles_2001-2100.tif", overwrite=T)

#### FINALEMENT CE N'EST PAS GENENANT D'INCLURE A L'ANALYSE LES ZONES ACTUELLEMEN CULTIVEES
### MAIS POUR AVOIR DES DONNEES CHIFFREES, ON CALCULE ET ON CARTOGRAPHIE EGAELEMENT 
#### LES ZONES  POTENTIELLES QUI NE SONT PAS DEJA PLANTEES EN VIGNE 
##### ZONES POTENTIELLES NON DEJA PLANTEES EN VIGNE ######

# Carte illustation les zones viticoles potentielles dans le monde sans vignobles actuels
pdf("outmaps/CartesMondiale_ZonesPotentielles_SansVignoblesActuels.pdf", width = 10,height = 6)
...
# Cartes des zones potentielles par région
pdf("outmaps/Regional_ZonesPotentielles_SansVignoblesActuelsInclus.pdf", width=12, height = 6)
zoomtab <- read.csv("ZoomTable.csv")
for(i in 1:nrow(zoomtab))
{  
  myext <- ext(unlist(zoomtab[i,-1]))
