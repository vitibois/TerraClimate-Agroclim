# Exploratory analysis of agroclimatic features and Koppen climate classes
# at vineyard points (VGDB): all periods/scenarios extracted by script 07.
#
# Originally written by Benjamin Bois, 2023.
# Reworked 2025-07-31, B. Bois: generalised to the combined all-periods file.
...
setwd(file.path(data_root, "Extraction_TerraClimatPoints", paste0("VGDB_v", vgdb_version)))

# Read data set ---------
d <- as.data.frame(data.table::fread(input =
...
# The regional AVA Nappa Valley is identified as "AVA Napa Valley"
...
# Koppen classification analysis ------------
kop <- read_csv(file.path(sig_root, "world/WorldKoppen/koppen.info.csv")) |>
...
# Proportion of regions with a very dry climate (DI < -100)
...
pdf("Climagram_IndHuglin-Secheresse.pdf", width = 8.5, height = 7)
  ...
  # Blank plot (no region colouring)
